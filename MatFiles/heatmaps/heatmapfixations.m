    ...
    try
        ...
        for stimIdx = 1:length(bottomUpStimuli)
            ...
            try
                ...
                for i = 1:length(events)
                    if strcmp(events(i).codestring, 'STARTFIX') && ...
                       events(i).sttime >= startTime && events(i).sttime <= endTime
                        startFixIdx(end+1) = i;
                    elseif strcmp(events(i).codestring, 'ENDFIX') && ...
                           events(i).sttime >= startTime && events(i).sttime <= endTime
                        endFixIdx(end+1) = i;
                    end
                end
                ...
                validPairs = [];
                for startIdx = 1:length(startFixIdx)
                    startFixTime = events(startFixIdx(startIdx)).sttime;
                    
                    for endIdx = 1:length(endFixIdx)
                        endFixTime = events(endFixIdx(endIdx)).sttime;
                        if endFixTime > startFixTime
                            % Check if both STARTFIX and ENDFIX are within our time window
                            if startFixTime >= startTime && endFixTime <= endTime
                                validPairs(end+1, :) = [startFixIdx(startIdx), endFixIdx(endIdx)];
                            end
                            break;
                        end
                    end
                ...
                if isempty(validPairs)
                    fprintf('  Warning: No valid fixation pairs found for stimulus %d in EDF %d\n', stimNum, edfNumber);
                    continue;
                end
                ...
                for pairIdx = 1:size(validPairs, 1)
                    startFixEvent = events(validPairs(pairIdx, 1));
                    endFixEvent = events(validPairs(pairIdx, 2));
                    
                    x = double(startFixEvent.gstx);
                    y = double(startFixEvent.gsty);
                    duration = double(endFixEvent.entime) - double(startFixEvent.sttime);

                    if x > 0 && x <= screenWidth && y > 0 && y <= screenHeight && ...
                       ~isnan(x) && ~isnan(y) && duration > 0
                        fixX(end+1) = x;
                        fixY(end+1) = y;
                        fixDuration(end+1) = duration;
                    end
                end
                
                fprintf('  Found %d valid fixations for stimulus %d\n', length(fixX), stimNum);
                ...
                if ~isempty(fixX)
                    ...
                    for i = 1:length(fixX)
                        ...
                        if x >= 1 && x <= screenWidth && y >= 1 && y <= screenHeight
                            heatmapMatrix(y, x) = heatmapMatrix(y, x) + fixDuration(i);
                        end
                    end
                    ...
                    if ~isempty(stimulusImagePath) && exist(stimulusImagePath, 'file')
                        try
                            ...
                            title(sprintf('Bottom-Up: EDF %d - Stimulus %d (%d fixations) - Overlay', ...
                                edfNumber, stimNum, length(fixX)), 'FontSize', 14);
                            ...
                            axis image off;
                            colormap jet;
                            title(sprintf('Bottom-Up: EDF %d - Stimulus %d (%d fixations) - Heatmap Only', ...
                                edfNumber, stimNum, length(fixX)), 'FontSize', 14);
                        end
                    else
                        imagesc(log(heatmapBlurred + 1));
                    ...
                    if ~exist('Results/Bottom-Up', 'dir'), mkdir('Results/Bottom-Up'); end
                    
                    outputName = sprintf('Results/Bottom-Up/%d-%d_overlay.png', edfNumber, stimNum);
                    saveas(gcf, outputName, 'png');
                    
                    print(gcf, sprintf('Results/Bottom-Up/%d-%d_overlay_hires.png', edfNumber, stimNum), ...
                        '-dpng', '-r300');
                ...
                if ~isempty(fixX)
                    ...
                    if ~isempty(stimulusImagePath) && exist(stimulusImagePath, 'file')
                        try
                            ...
                            title(sprintf('Top-Down: EDF %d - Stimulus %d (%d fixations) - Overlay', ...
                                edfNumber, stimNum, length(fixX)), 'FontSize', 14);
                            ...
                            axis image off;
                            colormap jet;
                            title(sprintf('Top-Down: EDF %d - Stimulus %d (%d fixations) - Heatmap Only', ...
                                edfNumber, stimNum, length(fixX)), 'FontSize', 14);
                        end
                    else
                        imagesc(log(heatmapBlurred + 1));
